...
def check_for_updates(callback=None):
    """Проверить наличие обновлений на GitHub. Вызывается в фоновом потоке."""
    try:
        ...
        if latest and latest != current:
            # Найти ссылку на скачивание для текущей ОС
            ...
            for asset in assets:
                ...
                elif os_name == "darwin" and (name.endswith(".dmg") or name.endswith(".app.zip")):
                    download_url = asset.get("browser_download_url", "")
...
class SettingsWindow(ctk.CTkToplevel):
    def __init__(self, parent, settings, on_save):
        ...
        self.title("Настройки"); self.geometry("540x620"); self.resizable(False, False)
        ...
        ctk.CTkButton(btn_inner, text="Отмена", width=120, height=38, corner_radius=10,
            fg_color="transparent", border_width=1, border_color=C["border"],
            text_color=C["text2"], command=self.destroy).pack(side="left")
        ctk.CTkButton(btn_inner, text="Сохранить", width=120, height=38, corner_radius=10,
    ...
    def _build(self):
        ...
        # API
        self._lbl("API ключ"); self._hint("Ключ от ProxyAPI, OpenAI или другого провайдера")
        ...
        self._tbtn = ctk.CTkButton(f, text="Показать", width=80, height=22, corner_radius=6,
            fg_color="transparent", border_width=1, border_color=C["border"],
            text_color=C["text2"], font=ctk.CTkFont(size=10), command=self._toggle)
        self._tbtn.pack(anchor="e", padx=P, pady=(2, 6))

        # Model
        self._lbl("Модель"); self._hint("Выберите или введите название")
        ...
        # URL
        self._lbl("Base URL"); self._hint("Выберите или введите URL")
        ...
        self._hint("0 = точный, 1 = креативный. Авто для reasoning-моделей")
        ...
        # Bubble color
        self._lbl("Цвет ваших сообщений")
    ...
    def _toggle(self):
        ...
        self._tbtn.configure(text="Скрыть" if self._show_key else "Показать")
    def _pick_dir(self):
...
class ChatApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(nc_frame, text="＋  Новый чат", font=self._fb(-2),
        ...
        self.search_entry = ctk.CTkEntry(self.sidebar, placeholder_text="🔍 Поиск...",
        ...
        ctk.CTkButton(sb_bottom, text="📂 Файлы", height=28, corner_radius=8,
        ...
        self.status_text = ctk.CTkLabel(top, text="Не подключен",
        ...
        def _input_menu(event):
            ...
            menu.add_command(label="Вставить", command=lambda: self._paste_to_input())
            try:
                self.input_box._textbox.get("sel.first", "sel.last")
                menu.add_command(label="Копировать", command=lambda: self._copy_from_input())
                menu.add_command(label="Вырезать", command=lambda: self._cut_from_input())
            except Exception:
                pass
            menu.add_separator()
            menu.add_command(label="Выделить всё", command=lambda: self._select_all_input())
        ...
        ctk.CTkLabel(self.ibar,
            text="Enter — отправить  •  Shift+Enter — перенос  •  📎 — файл  •  /help",
    ...
    def _on_update_check(self, info):
        ...
        # Баннер обновления
        banner = ctk.CTkFrame(self.right, fg_color=C["accent"], corner_radius=0, height=32)
        banner.pack(fill="x", after=list(self.right.winfo_children())[0])
        banner.pack_propagate(False)

        ctk.CTkLabel(banner, text=f"🔄 Доступна версия {latest}",
            font=self._fb(-3), text_color="#FFF").pack(side="left", padx=12)

        def _download():
            import webbrowser
            webbrowser.open(url)

        ctk.CTkButton(banner, text="Скачать", width=80, height=24, corner_radius=6,
            fg_color="#FFFFFF", hover_color="#E0E0E0", text_color=C["accent"],
            font=self._fb(-4), command=_download).pack(side="right", padx=(0, 6), pady=4)

        ctk.CTkButton(banner, text="✕", width=24, height=24, corner_radius=4,
            fg_color="transparent", hover_color="#FFFFFF33", text_color="#FFF",
            font=self._f(-4), command=banner.destroy).pack(side="right", padx=2, pady=4)
    ...
    def _refresh_sidebar(self):
        ...
        if not chats:
            txt = "Ничего не найдено" if query else "Нет чатов"
    ...
    def _save_current(self):
        ...
        if self.chat_title == "Новый чат" and self.chat_messages:
    ...
    def _show_welcome(self):
        ...
        ctk.CTkLabel(w, text="AI-ассистент: поиск, Excel, PDF, Word, изображения, код",
                      font=self._f(-2), text_color=C["text3"]).pack(pady=(0, 18))
        cards = ctk.CTkFrame(w, fg_color="transparent"); cards.pack()
        for emoji, txt in [("📊","Excel"), ("📄","PDF/Word"), ("🖼","Картинки"), ("🐍","Код")]:
            ...
            ctk.CTkLabel(c, text=txt, font=self._f(-5), text_color=C["text2"]).pack()
    ...
    def _add_msg_w(self, text, sender="user", files=None):
        ...
        name = "Вы" if is_user else "Ассистент"
        ...
        def _ctx_menu(event):
            ...
            try:
                ...
                sel = tb.get("sel.first", "sel.last")
                tb.configure(state="disabled")
                if sel: menu.add_command(label="Копировать выделенное", command=_copy_sel)
            except Exception: pass
            menu.add_command(label="Копировать всё", command=_copy_all)
            menu.tk_popup(event.x_root, event.y_root)
    ...
    def _show_streaming_bubble(self):
        ...
        ctk.CTkLabel(header, text="Ассистент", font=self._fb(-4),
        ...
        self._stream_tb.insert("1.0", "✦ Думаю...")
    ...
    def _append_stream(self, chunk):
        ...
        self._stream_tb.insert("1.0", display or "✦ Думаю...")
        self._stream_tb.configure(state="disabled")
    ...
    def _init_agent(self):
        ...
        if not key: self._sys_msg("⚠️ Настройки → API-ключ"); return False
        ...
        try:
            ...
            self.status_text.configure(text="Подключен", text_color=C["ok"])
            self.model_pill.configure(text=f"  {model_id}  ")
            self._sys_msg(f"✓ Агент готов  •  {model_id}"); return True
    ...
    def _on_saved(self, s):
        ...
        if (s.get("model") != old.get("model") or s.get("base_url") != old.get("base_url") or
            ...
            self.agent = None
            self.status_dot.configure(text_color=C["warn"])
            self.status_text.configure(text="Переподключение", text_color=C["warn"])
            self.model_pill.configure(text=f"  {s.get('model','?')}  ")
            self._sys_msg("Настройки обновлены")
        if s.get("theme") != old.get("theme"):
            self._sys_msg("Тема — перезапустите приложение")
        if s.get("bubble_color") != old.get("bubble_color"):
            self._sys_msg(f"Цвет: {s.get('bubble_color')}")
        if s.get("font_size") != old.get("font_size"):
            self._sys_msg(f"Шрифт: {s.get('font_size')} px — новые сообщения будут с новым размером")

    def _new_chat(self):
        self._save_current()
        self.chat_id = uuid.uuid4().hex[:12]
        self.chat_title = "Новый чат"; self.chat_messages = []
        for w in self.chat_scroll.winfo_children(): w.destroy()
        self._show_welcome()
        if self.agent:
            try:
